...
const getMissingSmtpKeys = () => (
    ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASS"].filter((key) => !process.env[key])
);
...
router.post("/register", async (req, res) => {
    try {
        ...
        const fullName = `${String(firstName || "").trim()} ${String(secondName || "").trim()}`.trim();

        if (!firstName || !secondName || !phone || !gender || !email || !password) {
            return res.status(400).json({
                message: "First name, second name, phone, gender, email and password are required"
            });
        }
...
router.get("/profile", authMiddleware, async (req, res) => {
    try {
        const user = await User.findById(req.user.userId).select("-password -resetCode -resetCodeExpires -resetCodeVerified");
...
router.post("/reset-password", async (req, res) => {
    try {
        ...
        if (!user || !user.resetCodeVerified || user.resetCodeExpires < new Date()) {
            return res.status(400).json({
